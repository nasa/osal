...
The `queue-test` unit test runs the timeout test twice:

- `QueueTimeoutTest` runs without realtime clock jumps.
- `QueueTimeoutTimeJumpTest` runs with a realtime "jumper" task when executed as root; otherwise it is
  skipped with a WARN result.

Note that Linux `mqueue` limits apply (e.g. `/proc/sys/fs/mqueue/msg_max`,
