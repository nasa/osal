...
int32 OS_API_Impl_Init(osal_objtype_t idtype)
{
    ...
    return_code = OS_QT_TableMutex_Init(idtype);
    ...
    switch (idtype)
    {
        case OS_OBJECT_TYPE_OS_TASK:
            return_code = OS_QT_TaskAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_QUEUE:
            return_code = OS_QT_QueueAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_BINSEM:
            return_code = OS_QT_BinSemAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_COUNTSEM:
            return_code = OS_QT_CountSemAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_MUTEX:
            return_code = OS_QT_MutexAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_MODULE:
            return_code = OS_QT_ModuleAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_TIMEBASE:
            return_code = OS_QT_TimeBaseAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_STREAM:
            return_code = OS_QT_StreamAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_DIR:
            return_code = OS_QT_DirAPI_Impl_Init();
            break;
        case OS_OBJECT_TYPE_OS_FILESYS:
            return_code = OS_QT_FileSysAPI_Impl_Init();
            break;
        default:
            break;
    }
...
void OS_IdleLoop_Impl(void)
{
     ...
     * to keep them masked here (this is different than the original QT impl).  The
     * timebase objects have a dedicated thread that will be doing "sigwait" on those.
     */
    sigsuspend(&QT_GlobalVars.NormalSigMask);
