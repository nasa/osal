...
/****************************************************************************************
                                   GLOBAL DATA
 ***************************************************************************************/
const char OS_QT_DEVICEFILE_PREFIX[] = "/dev/";
...
/* --------------------------------------------------------------------------------------
    Name: OS_QT_FileSysAPI_Impl_Init
...
{
    return OS_SUCCESS;
} /* end OS_QT_FileSysAPI_Impl_Init */
...
int32 OS_FileSysStartVolume_Impl(const OS_object_token_t *token)
{
    ...
    if (local->fstype == OS_FILESYS_TYPE_UNKNOWN &&
        strncmp(local->device_name, OS_QT_DEVICEFILE_PREFIX, sizeof(OS_QT_DEVICEFILE_PREFIX) - 1) == 0)
    {
        /*
...
int32 OS_FileSysFormatVolume_Impl(const OS_object_token_t *token)
{
     ...
     * (this is also backward compatible since QT mkfs was always a no-op)
     */
    return OS_SUCCESS;
...
int32 OS_FileSysMountVolume_Impl(const OS_object_token_t *token)
{
     ...
     * mount a disk, if warranted.  For all current QT-based PSPs,
...
{
    /*
     * NOTE: Mounting/Unmounting on QT is not implemented.
